      ...
      <nav className="w-full py-6 px-4 sm:px-6 lg:px-8 border-b border-gray-100">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <a href="/" className="text-gray-500 hover:text-brand-navy text-sm font-medium transition-colors">{t('auth.home')}</a>
            <a href="/#contact" className="text-gray-500 hover:text-brand-navy text-sm font-medium transition-colors">{t('auth.contact')}</a>
      ...
      <main className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8" style={{ animation: 'fadeIn 0.8s ease-out' }}>
        <div className="max-w-md w-full space-y-10">
          ...
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-serif text-brand-navy mb-4">{t('auth.createAnAccount')}</h1>
            <p className="text-gray-500 text-sm">{t('auth.joinPortal')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="name" className="block text-sm font-bold text-brand-navy">{t('auth.fullName')}*</label>
              <input 
                type="text" 
                id="name" 
            ...
            {/* Repeat Password */}
            <div className="space-y-2">
              <label htmlFor="confirm-password" className="block text-sm font-bold text-brand-navy">{t('auth.repeatPassword')}*</label>
              <input 
                type="password" 
                id="confirm-password" 
            ...
            <div className="space-y-4 pt-2">
              ...
              >
                ...
                <span className="group-hover:text-brand-slate">{t('auth.signUpWithGoogle')}</span>
              </button>
            </div>
          ...
          <div className="text-center pt-4">
            <p className="text-sm text-gray-500">
              {t('auth.haveAccount')}
              <a href="/signin" className="text-brand-gold font-bold hover:text-brand-goldLight underline decoration-2 underline-offset-4 transition-colors ml-1">{t('auth.signIn')}</a>
            </p>
          </div>
